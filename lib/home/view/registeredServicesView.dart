                            ...
                            child: Card(
                              ...
                              child: ListTile(
                                leading: CachedNetworkImage(
                                  height: 100,
                                  width: 100,
                                  imageUrl: snapshot.data![index].data().toString().contains("uploadImage")? snapshot.data![index]["uploadImage"]:"https://images.pexels.com/photos/13519033/pexels-photo-13519033.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"),
                                title: Text("Name "+snapshot.data?[index]["name"]),
                                subtitle: Text("Contact :"+ snapshot.data?[index]["contact"]),
                                trailing: IconButton(onPressed: () {
                                  servicesController.deleteUserService(serviceType,index);
  ...
  Future<List<DocumentSnapshot>> getStream() async{
    ...
    if(serviceType == ServiceType.HOSTEL){
      for (int i=0;i<servicesController.userHostel.length;i++){
        data.add(await db.collection("hostel").doc(servicesController.userHostel[i]).get()); 
      }
    }
    else if(serviceType == ServiceType.MESS){
      for (int i=0;i<servicesController.userMess.length;i++){
        data.add(await db.collection("mess").doc(servicesController.userMess[i]).get()); 
      }
    }
    else if(serviceType == ServiceType.SALON){
      for (int i=0;i<servicesController.userSalon.length;i++){
        data.add(await db.collection("salon").doc(servicesController.userSalon[i]).get()); 
      }
    }
    else{
      for (int i=0;i<servicesController.userStationery.length;i++){
        data.add(await db.collection("stationery").doc(servicesController.userStationery[i]).get()); 
      }
    }
